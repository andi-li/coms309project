...
import com.andi.DungeonExplorer.model.world.Items.Bow;
...
public class World {
	...
	public void addTiles(int startX, int startY, int width, int height, int type){
		int i,j;
		TERRAIN terrain = TERRAIN.GRASS_0;
		if(type == 1){
			terrain = TERRAIN.ice;
		}
		if(type == 2){
	...
	public void addTree(int x, int y) {
		...
		WorldObject tree = new WorldObject(x, y, false, treeRegion, 2f, 3f, gridArray,true);
		this.addObject(tree);
	}
    ...
    public void add6x5SnowTreeRegion(int x, int y) {
        ...
        WorldObject tree = new WorldObject(x, y, false, treeRegion, 6f, 5f, gridArray,true);
        this.addObject(tree);
    }
    ...
    public void add4x19SnowTreeRegion(int x, int y) {
        ...
        WorldObject tree = new WorldObject(x, y, false, treeRegion, 4f, 19f, gridArray,true);
        this.addObject(tree);
    }
    ...
    public void addHouse( int x, int y) {
		...
		WorldObject house = new WorldObject(x, y, false, houseRegion, 5f, 5f, gridArray,true);
	...
	public void addRug(World world, int x, int y) {
		...
		WorldObject rug = new WorldObject(x, y, true, rugRegion, 3f, 2f, gridArray,true);
		world.addObject(rug);
	}
	...
	public void addVisualObject(int x,int y,String name,boolean walkable,float sizeX,float sizeY){
		...
		WorldObject obj = new WorldObject(x,y,walkable,region,sizeX,sizeY,gridArray,true);
		this.addObject(obj);
	}
	...
	public WorldObject createObject(int x,int y,boolean walkable, String name,float sizeX,float sizeY,boolean visible){
		TextureAtlas atlas = assetManager.get("res/graphics_packed/tiles/tilepack.atlas", TextureAtlas.class);
		TextureRegion region = atlas.findRegion(name);
		WorldObject obj;
		if(walkable){
			obj = new WorldObject(x,y,walkable,region,sizeX,sizeY,new GridPoint2(0,0),visible);
		}
		else{
			GridPoint2[] gridArray = new GridPoint2[(int)sizeX*(int)sizeY];
			int index = 0;
			...
			obj = new WorldObject(x,y,walkable,region,sizeX,sizeY,gridArray,visible);
			return obj;
		}
		return obj;
	}
